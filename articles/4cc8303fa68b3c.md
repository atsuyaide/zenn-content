...
_*注釈: Docker が動作すればよいので, Mac や Linux 環境でも問題ありません.*_
...
また, [各種 UI ツールを試す](#🕶-各種-ui-ツールを試す)のセクションで起動したツールを使うと Kafka に保存されているメッセージの詳細やコンシューマの状態なども確認できるものもあります. 例えば, `--group-id`でコンシューマーグループ ID を指定することで Kafka UI のコンシューマ一覧に表示されるようになります.
